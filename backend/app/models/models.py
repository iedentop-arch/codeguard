...
class Vendor(Base, TimestampMixin):
    ...
    contract_start: Mapped[datetime | None] = mapped_column(Date, comment="合同开始日期")
    contract_end: Mapped[datetime | None] = mapped_column(Date, comment="合同结束日期")
    github_org: Mapped[str | None] = mapped_column(String(100), comment="GitHub组织")
    current_grade: Mapped[SLAGrade | None] = mapped_column(SQLEnum(SLAGrade), comment="当前等级")
    current_score: Mapped[float | None] = mapped_column(Numeric(5, 1), comment="当前评分")
...
class VendorMember(Base, TimestampMixin):
    ...
    user_id: Mapped[str | None] = mapped_column(String(64), comment="用户ID")
    ...
    github_username: Mapped[str | None] = mapped_column(String(50), comment="GitHub用户名")
    exam_score: Mapped[int | None] = mapped_column(Integer, comment="考试分数")
    exam_attempts: Mapped[int] = mapped_column(Integer, default=0, comment="考试尝试次数")
    certification_id: Mapped[str | None] = mapped_column(String(32), comment="认证编号")
    is_deleted: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    # Phase 3: GitHub权限字段
    github_permission_status: Mapped[str] = mapped_column(String(20), default="none", comment="GitHub权限状态")
    github_permission_level: Mapped[str | None] = mapped_column(String(10), comment="GitHub权限级别")
    github_permission_synced_at: Mapped[datetime | None] = mapped_column(DateTime, comment="权限同步时间")
...
class SpecDocument(Base, TimestampMixin):
    ...
    file_path: Mapped[str | None] = mapped_column(String(300), comment="文件路径(如02-architecture/system-design-principles.md)")
...
class ExamQuestion(Base, TimestampMixin):
    ...
    rule_id: Mapped[str | None] = mapped_column(String(20), comment="规则编号")
    ...
    code_snippet: Mapped[str | None] = mapped_column(Text, comment="代码片段")
...
class PullRequest(Base, TimestampMixin):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    vendor_id: Mapped[int] = mapped_column(Integer, ForeignKey("vendors.id"), nullable=False)
    author_id: Mapped[int | None] = mapped_column(Integer, comment="作者ID")
    github_pr_number: Mapped[int] = mapped_column(Integer, nullable=False, comment="PR编号")
    github_pr_url: Mapped[str | None] = mapped_column(String(500), comment="PR链接")
    head_sha: Mapped[str | None] = mapped_column(String(64), comment="PR head commit SHA")
    title: Mapped[str] = mapped_column(String(200), nullable=False, comment="标题")
    branch: Mapped[str | None] = mapped_column(String(100), comment="分支名")
    ...
    merged_at: Mapped[datetime | None] = mapped_column(DateTime, comment="合并时间")
    is_deleted: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
...
class QualityGate(Base, TimestampMixin):
    ...
    details: Mapped[str | None] = mapped_column(Text, comment="详情(JSON)")
...
class MonthlyScore(Base, TimestampMixin):
    ...
    warning_trend_pct: Mapped[float | None] = mapped_column(Numeric(5, 1), comment="WARNING趋势")
...
class Delivery(Base, TimestampMixin):
    ...
    artifacts: Mapped[str | None] = mapped_column(Text, comment="附件(JSON)")
    status: Mapped[str] = mapped_column(String(20), default="submitted", comment="状态")
    reviewed_at: Mapped[datetime | None] = mapped_column(DateTime, comment="评审时间")
    reviewed_by: Mapped[str | None] = mapped_column(String(64), comment="评审人")
    reviewer_notes: Mapped[str | None] = mapped_column(Text, comment="评审备注")
    is_deleted: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
...
class DeliveryChecklist(Base, TimestampMixin):
    ...
    evidence_url: Mapped[str | None] = mapped_column(String(500), comment="证据链接")
    reviewer_notes: Mapped[str | None] = mapped_column(Text, comment="评审备注")
...
class SpecVersion(Base):
    ...
    content_snapshot: Mapped[str | None] = mapped_column(Text, comment="内容快照")
    ...
    changed_by: Mapped[int | None] = mapped_column(Integer, ForeignKey("users.id"), comment="变更人")
...
class SpecChangeNotification(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    spec_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("spec_documents.id"))
    old_version: Mapped[str | None] = mapped_column(String(20), comment="旧版本")
    new_version: Mapped[str | None] = mapped_column(String(20), comment="新版本")
    change_type: Mapped[str | None] = mapped_column(String(20), comment="变更类型")
    change_summary: Mapped[str | None] = mapped_column(Text, comment="变更摘要")
...
class NotificationRecord(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    notification_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("spec_change_notifications.id"))
    vendor_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("vendors.id"))
    member_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("vendor_members.id"))
    channel: Mapped[str | None] = mapped_column(String(20), comment="通知渠道")
    status: Mapped[str | None] = mapped_column(String(20), comment="发送状态")
...
class QuarterlyTraining(Base, TimestampMixin):
    ...
    title: Mapped[str | None] = mapped_column(String(200), comment="培训标题")
    description: Mapped[str | None] = mapped_column(Text, comment="培训描述")
    spec_ids: Mapped[str | None] = mapped_column(Text, comment="关联规范ID(JSON)")
    start_date: Mapped[datetime | None] = mapped_column(Date, comment="开始日期")
    end_date: Mapped[datetime | None] = mapped_column(Date, comment="结束日期")
...
class TrainingRecord(Base, TimestampMixin):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    quarterly_training_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("quarterly_trainings.id"))
    vendor_member_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("vendor_members.id"))
    vendor_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("vendors.id"))
    status: Mapped[str] = mapped_column(String(20), default="enrolled", comment="状态")
    exam_score: Mapped[int | None] = mapped_column(Integer, comment="考试分数")
    exam_passed: Mapped[bool | None] = mapped_column(Boolean, comment="是否通过")
    certification_id: Mapped[str | None] = mapped_column(String(32), comment="认证编号")
    enrolled_at: Mapped[datetime | None] = mapped_column(DateTime, comment="报名时间")
    completed_at: Mapped[datetime | None] = mapped_column(DateTime, comment="完成时间")
...
class AuditLog(Base):
    ...
    user_name: Mapped[str | None] = mapped_column(String(50), comment="用户名")
    ...
    vendor_id: Mapped[int | None] = mapped_column(Integer, comment="涉及供应商")
    old_value: Mapped[str | None] = mapped_column(Text, comment="旧值(JSON)")
    new_value: Mapped[str | None] = mapped_column(Text, comment="新值(JSON)")
...
class SLAAppeal(Base, TimestampMixin):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    vendor_id: Mapped[int] = mapped_column(Integer, ForeignKey("vendors.id"), nullable=False)
    submitter_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("vendor_members.id"))
    monthly_score_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("monthly_scores.id"))
    appeal_type: Mapped[str | None] = mapped_column(String(20), comment="申诉类型")
    reason: Mapped[str | None] = mapped_column(Text, comment="申诉理由")
    evidence_urls: Mapped[str | None] = mapped_column(Text, comment="证据链接(JSON)")
    status: Mapped[str] = mapped_column(String(20), default="pending", comment="状态")
    ...
    review_notes: Mapped[str | None] = mapped_column(Text, comment="审核备注")
...
class SystemConfig(Base, TimestampMixin):
    ...
    config_type: Mapped[str | None] = mapped_column(String(20), comment="配置类型")
    description: Mapped[str | None] = mapped_column(Text, comment="配置描述")
    category: Mapped[str | None] = mapped_column(String(50), comment="配置分类")
    is_active: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
